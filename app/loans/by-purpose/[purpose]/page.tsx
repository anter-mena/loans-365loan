...
import { LOAN_PURPOSES, getLoanPurpose } from "@/lib/loans/purposes";
import { getLoanAmount } from "@/lib/loans/amounts";
...
const LAST_UPDATED_DISPLAY = "July 6, 2026";

export function generateStaticParams() {
  return LOAN_PURPOSES.map((p) => ({ purpose: p.slug }));
}
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const purpose = getLoanPurpose(slug);
  if (!purpose) return {};

  return buildMetadata({
    title: `${purpose.title} Loans in Canada | 365loan`,
    description: purpose.description,
    path: `/loans/by-purpose/${purpose.slug}`,
    keywords: [`${purpose.title.toLowerCase()} loan canada`, "personal loan canada", `${purpose.title.toLowerCase()} financing`],
  });
}

export default async function LoanPurposePage({ params }: { params: Params }) {
  const { purpose: slug } = await params;
  const purpose = getLoanPurpose(slug);
  if (!purpose) notFound();

  const { title, description, intro, covers, considerations, suggestedAmountSlug, faqs: uniqueFaqs } = purpose;
  const suggestedAmount = getLoanAmount(suggestedAmountSlug);
  ...
  const breadcrumb = breadcrumbJsonLd([
    ...
    { name: `${title} Loan`, path: `/loans/by-purpose/${slug}` },
  ]);
  ...
  const articleJsonLd = {
    "@context": "https://schema.org",
    "@type": "Article",
    headline: `${title} Loans in Canada`,
    description,
    ...
    mainEntityOfPage: `${BASE_URL}/loans/by-purpose/${slug}`,
  };

  return (
    <>
      <Script id={`purpose-${slug}-breadcrumb`} type="application/ld+json" strategy="afterInteractive">
        {JSON.stringify(breadcrumb)}
      </Script>
      <Script id={`purpose-${slug}-faq`} type="application/ld+json" strategy="afterInteractive">
        {JSON.stringify(faqJsonLd)}
      </Script>
      <Script id={`purpose-${slug}-article`} type="application/ld+json" strategy="afterInteractive">
      ...
      <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-[720px] mx-auto px-4 lg:px-8 flex flex-col items-center text-center">
          ...
          <h1 className="font-heading font-black text-[2.25rem] lg:text-[3rem] text-foreground leading-[1.1] tracking-tight mb-5">
            {title} <span className="text-primary">Loans</span> in Canada
          </h1>
        ...
        <article className="max-w-[760px] mx-auto px-4 lg:px-8">
          ...
          <div className="mb-14">
            <h2 className="font-heading font-bold text-[1.3rem] text-foreground mb-3">
              Using a Personal Loan for {title}
            </h2>
            <p className="text-[0.9rem] text-muted-foreground leading-relaxed">
              {intro} It's one of several{" "}
              <a href="/loans/by-purpose" className="text-primary font-semibold underline underline-offset-2">
                reasons Canadians borrow
              </a>{" "}
          ...
          {/* Suggested amount cross-link */}
          {suggestedAmount && (
            <div className="mb-14 relative overflow-hidden bg-navy rounded-2xl p-8 flex flex-col sm:flex-row sm:items-center gap-6">
              ...
              <div className="relative z-10 w-14 h-14 rounded-full bg-primary/15 flex items-center justify-center shrink-0">
                <Banknote size={24} className="text-primary" strokeWidth={1.75} />
              </div>
              <div className="relative z-10 flex-1">
                <h2 className="font-heading font-bold text-[1.1rem] text-white mb-1.5">
                  How Much Do You Typically Need?
                </h2>
                <p className="text-[0.875rem] text-white/55 leading-relaxed">
                  For {title.toLowerCase()}, borrowers often look at around {suggestedAmount.display} — though the right amount depends on your specific situation.
                </p>
              </div>
              <a
                href={`/loans/by-amount/${suggestedAmount.slug}`}
                className="relative z-10 inline-flex items-center gap-1.5 bg-white text-ink text-[0.825rem] font-semibold px-5 py-3 rounded-full shrink-0 transition-transform hover:translate-x-1"
              >
                See {suggestedAmount.display} loan details
            ...
            <div>
              ...
              <ul className="flex flex-col gap-3">
                {considerations.map((item) => (
                  <li key={item} className="flex items-start gap-2.5 text-[0.875rem] text-muted-foreground leading-relaxed">
                    <AlertCircle size={16} className="text-muted-foreground shrink-0 mt-0.5" strokeWidth={1.75} />
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="bg-card border border-border rounded-2xl p-8 lg:p-9">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
              {STEP_ICONS.map((Icon, i) => {
                const step = [
                  { title: "Apply in 2 minutes", description: `Tell us you're covering ${title.toLowerCase()} and a bit about your situation. No paperwork.` },
      ...
      <section className="bg-background py-20 lg:py-28">
        <div className="max-w-[760px] mx-auto px-4 lg:px-8">
          <h2 className="font-heading font-black text-[1.6rem] lg:text-[2rem] text-foreground leading-[1.1] tracking-tight mb-8 text-center">
            <span className="text-primary">{title} Loan</span> FAQ
          </h2>
